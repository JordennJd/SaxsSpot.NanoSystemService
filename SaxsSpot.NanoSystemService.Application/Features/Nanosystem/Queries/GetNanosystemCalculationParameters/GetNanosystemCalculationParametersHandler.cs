...
public class GetNanosystemCalculationParametersHandler(INanoSystemStorage storage, INanoSystemObjectStorage objectStorage,
    ...
    : IRequestHandler<GetNanosystemCalculationParametersQuery, IResult<NanosystemCalculationParametersDto>>
{
    public async Task<IResult<NanosystemCalculationParametersDto>> Handle(GetNanosystemCalculationParametersQuery request,
        CancellationToken cancellationToken)
    {
        logger.Log(LogLevel.Information, $"requested NanosystemCalculationParameters with nanosystemId: {request.NanosystemId}");
        ...
        await foreach (var particle in particles)
        {
            if (particle is Parallelepiped parallelepiped)
            {
                ...
                amplitudes[index] = new Amplitude([parallelepiped.X, parallelepiped.Y, parallelepiped.Z], parallelepiped.GetAmplitude());
                index++;
            }
        }
        
        return FluentResults.Result
            .Ok(new NanosystemCalculationParametersDto(pariclesVolume, amplitudes, nanosystem.GlobalSize));
    }
